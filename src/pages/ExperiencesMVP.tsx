...
const emptyForm: ExperienceForm = {
  ...
  context: "", humanFactors: "", deviations: "", actionTaken: "", learning: "",
};
...
async function fetchExperiences(companyId: string): Promise<Experience[]> {
  ...
  return (data ?? []).map((r: any): Experience => ({
    id: r.id,
...
async function upsertExperience(
  ...
  if (error) { console.error("[db] upsertExperience", error); return false; }
  return true;
}

async function deleteExperience(id: string): Promise<boolean> {
  const { error } = await sb.from("experiences").delete().eq("id", id);
  if (error) { console.error("[db] deleteExperience", error); return false; }
  return true;
}
...
export default function ExperiencesMVPPage() {
  ...
  const openEdit = (exp: Experience) => {
    setForm({ date: exp.date, context: exp.context, humanFactors: exp.humanFactors,
              deviations: exp.deviations, actionTaken: exp.actionTaken, learning: exp.learning });
    setEditingId(exp.id);
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    if (!form.context.trim()) {
      toast({ title: "Contexto é obrigatório", variant: "destructive" }); return;
    }
    setSaving(true);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (ok) { toast({ title: "Experiência excluída." }); await loadData(); }
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex gap-3 items-center">
          <div className="relative flex-1 max-w-md">
            ...
            <Input className="pl-9" placeholder="Buscar por contexto, fatores ou aprendizado..."
              value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <Button onClick={openAdd}><Plus size={14} className="mr-1" /> Nova Experiência</Button>
        </div>
              ...
              <Card key={exp.id} className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0 space-y-3">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Contexto</p>
                      <p className="text-sm text-foreground">{exp.context}</p>
                    </div>
                    {exp.humanFactors && (
                      <div>
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Fatores Humanos</p>
                        <p className="text-sm text-muted-foreground">{exp.humanFactors}</p>
                      </div>
                    )}
                    {exp.learning && (
                      <div>
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Aprendizado</p>
                        <p className="text-sm text-muted-foreground">{exp.learning}</p>
                      </div>
                    )}
                  </div>
                  <div className="flex gap-1 flex-shrink-0">
                    <Button variant="ghost" size="sm" onClick={() => openEdit(exp)}><Pencil size={14} /></Button>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
              ...
              <div className="space-y-1">
                ...
                <Textarea placeholder="Quais fatores humanos estavam presentes..." value={form.humanFactors} onChange={f("humanFactors")} rows={2} />
              </div>
              <div className="space-y-1">
                <Label>Desvios Observados</Label>
                <Textarea placeholder="Quais desvios foram identificados..." value={form.deviations} onChange={f("deviations")} rows={2} />
              </div>
              <div className="space-y-1">
                <Label>Ação Tomada</Label>
                <Textarea placeholder="O que foi feito..." value={form.actionTaken} onChange={f("actionTaken")} rows={2} />
              </div>
              <div className="space-y-1">
                <Label>Aprendizado</Label>
                <Textarea placeholder="O que foi aprendido..." value={form.learning} onChange={f("learning")} rows={2} />
              </div>
            </div>
            <DialogFooter>
