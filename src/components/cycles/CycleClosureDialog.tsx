...
async function avaliarCicloSupabase(companyId: string, cycleId: string): Promise<CycleEvaluationResult> {
  ...
  const hasDecisionOrValidation = cycleRecords.some(
    (r) => r.type === "decision" || r.type === "validation" || r.type === "meeting",
  );

  const actionCompletionPercent = totalActions > 0 ? Math.round((completedActions / totalActions) * 100) : 0;
  ...
  if (completedTurmas < 1) blockers.push("Nenhuma turma concluída (mínimo: 1)");
  if (!hasDecisionOrValidation) warnings.push("Sem registro de decisão ou validação associado ao ciclo");
  if (delayedActions > 0) warnings.push(`${delayedActions} ação(ões) ainda atrasada(s)`);
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <Icon
                          size={14}
                          className={item.met ? "text-success" : item.isBlocker ? "text-destructive" : "text-warning"}
                        />
