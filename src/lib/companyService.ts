...
function mapOnboardingFromDB(dbStatus: string | null | undefined): "not_started" | "in_progress" | "completed" {
...
function mapOnboardingToDB(appStatus: string): "nao_iniciado" | "em_andamento" | "concluido" {
...
export async function fetchCompanyById(companyId: string): Promise<CompanyState | null> {
...
export async function createCompanyInSupabase(company: CompanyState): Promise<{
  ...
  const { data, error } = await supabase.from("companies").insert(insertData).select("id").single();
...
export async function createCompanyAdmin(
  ...
  // 2b. Verify the company_id was actually saved — retry if not
  const { data: savedProfile } = await (supabase.from("profiles") as any)
    .select("company_id")
    .eq("id", userId)
    .single();

  if (!savedProfile?.company_id) {
    console.warn("Profile company_id not set, retrying...");
    await (supabase.from("profiles") as any).update({ company_id: companyId }).eq("id", userId);
  }
...
export async function deleteCompanyFromSupabase(companyId: string): Promise<boolean> {
  ...
  await (supabase.from("profiles") as any).update({ company_id: null }).eq("company_id", companyId);

  // Delete user_roles for users linked to this company
  const { data: linkedProfiles } = await supabase.from("profiles").select("id").eq("company_id", companyId);
  ...
  const { data, error } = await supabase.from("companies").delete().eq("id", companyId).select("id");
...
export async function updateCompanyInSupabase(
  ...
  const { error } = await (supabase.from("companies") as any).update(dbUpdates).eq("id", companyId);
...
export async function deactivateCompanyInSupabase(companyId: string): Promise<boolean> {
  return updateCompanyInSupabase(companyId, { active: false });
}

export async function reactivateCompanyInSupabase(companyId: string): Promise<boolean> {
